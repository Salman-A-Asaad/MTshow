...
let pageSearchMovies = 1;
let totalPageSearchMovies;
...
pageNumber.innerHTML = pageSearchMovies;
...
function findMovies() {
    fetch(
            `https://api.themoviedb.org/3/search/movie?query=${localStorage.getItem(
      "searchM"
    )}&page=${pageSearchMovies}`,
            options
        )
        .then((response) => response.json())
        .then((response) => {
            totalPageSearchMovies = response.total_pages;
            displaySearchMovies(response.results);
        })
...
function displaySearchMovies(data) {
    ...
    data.forEach((element) => {
        ...
        item += `<div class="col-sm-6 col-md-4 col-lg-3 pb-4">
            <div class="card bg-dark align-self-stretch">
              ...
              <div class="card-body">
                <h5 class="card-title text-light mb-3">${title}</h5>
                <a
                id="${id}"
                  href="#"
    ...
    if (data.length == 0) {
        ...
        row.innerHTML = `<h2 class="text-center text-light">No Results</h2>`;
        nextBtn.classList.add("disable");
    }
}

function disablePreBtn() {
    if (pageSearchMovies == 1) preBtn.classList.add("disable");
}

function disableNextBtn() {
    if (pageSearchMovies == totalPageSearchMovies)
        nextBtn.classList.add("disable");
}
// Trigger the fuction
findMovies();
...
nextBtn.onclick = function() {
    ...
    if (pageSearchMovies == 2) {
        preBtn.classList.remove("disable");
    }
};
preBtn.onclick = function() {
    if (pageSearchMovies > 1) {
        pageNumber.innerHTML = --pageSearchMovies;
        findMovies();
        disablePreBtn();
        window.scrollTo(0, 0);
    }
    if (pageSearchMovies == totalPageSearchMovies - 1) {
