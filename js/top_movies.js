...
pageNumber.innerHTML = pageTopMovies;
...
function findMovies() {
    fetch(
            `https://api.themoviedb.org/3/movie/top_rated?language=en-US&page=${pageTopMovies}`,
            options
        )
        .then((response) => response.json())
        .then((response) => {
            totalpageTopMovies = response.total_pages;
            displayTopMovies(response.results);
        })
...
function disablePreBtn() {
    if (pageTopMovies == 1) preBtn.classList.add("disable");
}

function disableNextBtn() {
    if (pageTopMovies == totalpageTopMovies) nextBtn.classList.add("disable");
}
...
nextBtn.onclick = function() {
    if (pageTopMovies < totalpageTopMovies) {
        pageNumber.innerHTML = ++pageTopMovies;
        findMovies();
        disableNextBtn();
        window.scrollTo(0, 0);
